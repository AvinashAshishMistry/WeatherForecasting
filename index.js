...
const currentHour = now.getHours('en-IN', { hour: '2-digit', hour12: true });
...
const currentTime = time.toISOString().slice(0, 16);
const baseTime = new Date(currentTime); // starting time

// Add 2 days and reset to 00:00
const target = new Date(baseTime);
target.setDate(target.getDate() + 1);
target.setHours(0, 0, 0, 0);

const offset = target.getTimezoneOffset() * 60000;
const nextMidnight = new Date(target - offset).toISOString().slice(0, 16);
...
async function weather(latitude, longitude) {
  ...
  console.log(visiAndUvindex)
  currentWeather(currentData, currentUnits, otherCurrents);

  hourlyWeather(hourlydata['hourly'], currentData["is_day"]);
  ...
  const archive = await fetch(`https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&start_date=2025-06-29&end_date=2025-06-29&daily=sunrise,sunset&timezone=auto`)
  ...
  sunSetRise(sunSetRiseData, previousDay);
}
...
document.addEventListener("DOMContentLoaded",deviceLocation)
...
////////////////////////////hourly weather ///////////////////////////////////////
function hourlyWeather(hourly, isDay) {

  let times = currentHour;
...
function sunSetRise(data, pData) {
  ...
  else if (!data[0] && currentTime < nextMidnight) {
    sm2.classList.value = "fa-regular fa-sunrise fa-sm sm1";
    sm1.classList.value = "fa-regular fa-sunset fa-sm sm2";
