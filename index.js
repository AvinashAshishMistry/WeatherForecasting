...
async function weather(latitude, longitude) {
  ...
  const Midnight = new Date(timeString);
  Midnight.setDate(Midnight.getDate() + 1);
  Midnight.setHours(0, 0, 0, 0);

  const n_Midnight = Midnight.getTimezoneOffset() * 60000;
  const nextMidnight = new Date(Midnight - n_Midnight).toISOString().slice(0, 16);
  ...
  sunSetRise(sunSetRiseData, previousDay, dateObj, timeString, nextMidnight);
}
...
function sunSetRise(data, pData, now, currentTime, nextMidnight) {
